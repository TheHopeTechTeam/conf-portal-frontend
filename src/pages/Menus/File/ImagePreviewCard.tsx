...
interface ImagePreviewCardProps {
  ...
  onClick?: () => void; // 點擊圖片容器事件（如果提供，則不會使用內建的放大功能）
  ...
  enableImagePreview?: boolean; // 是否啟用圖片放大預覽功能，默認為 true
}
...
const ImagePreviewCard = ({
  ...
  message,
  enableImagePreview = true, // 默認啟用圖片放大預覽
  ...
  return (
    <>
      <div className="flex flex-col">
        {/* 圖片容器 */}
        <div
          className={`group relative aspect-square overflow-hidden transition-all rounded-t-lg border-gray-200 dark:border-gray-700 cursor-pointer ${className}`}
        ...
        {fileInfo && (
          <div className="p-2 space-y-1 rounded-b-lg border-1 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
            ...
            {showUploadProgress && uploadStatus === "error" && (
              <p className="pt-2 text-[10px] text-center text-red-600 dark:text-red-400">{message || "上傳失敗"}</p>
            )}
            {showUploadProgress && uploadStatus === "success" && (
              <p className="pt-2 text-[10px] text-center text-green-600 dark:text-green-400">{message || "上傳完成"}</p>
            )}
          </div>
        )}
      </div>
      ...
      {enableImagePreview && (
        <Modal
          ...
          className="max-w-[95vw] max-h-[95vh] w-auto h-auto mx-4 p-6 shadow-none"
          showCloseButton={true}
          isFullscreen={false}
        >
          <div className="flex flex-col items-center justify-center bg-gray-600 dark:bg-gray-900 rounded-lg">
            {/* 放大圖片 */}
            <img
              src={imageUrl}
              alt={alt}
              className="max-w-full max-h-[calc(90vh-120px)] m-4 object-contain rounded-lg shadow-2xl bg-transparent"
              onClick={(e) => e.stopPropagation()}
            />
            {/* 圖片資訊 */}
            {fileInfo && (
              <div className="w-full bg-black/70 text-white p-4 rounded-b-lg backdrop-blur-sm">
                {fileInfo.name && (
                  <p className="text-sm font-medium truncate mb-1" title={fileInfo.name}>
                    {fileInfo.name}
                  </p>
                )}
                {(fileInfo.size !== undefined || fileInfo.type) && (
