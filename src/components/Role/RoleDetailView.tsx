  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DetailItem label="代碼" value={<code className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded">{role.code}</code>} />
        <DetailItem label="名稱" value={role.name || "-"} />
        <DetailItem label="啟用" value={role.isActive ? "是" : "否"} />
        <DetailItem label="建立時間" value={role.createAt || "-"} />
        <DetailItem label="更新時間" value={role.updateAt || "-"} />
      </div>

      <div>
        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">描述</div>
      ...
      <div>
        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">備註</div>
        <div className="rounded-lg border border-gray-200 p-3 text-sm text-gray-700 dark:border-gray-700 dark:text-gray-300 min-h-10">
          {role.remark || "-"}
        </div>
      </div>
      ...
      <div>
        <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">權限（{role.permissions?.length || 0}）</div>
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          {(role.permissions || []).length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">無</div>
          ) : (
