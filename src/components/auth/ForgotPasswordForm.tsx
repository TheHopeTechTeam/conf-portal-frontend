...
export default function ForgotPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || "發生錯誤，請稍後再試");
      }
    } catch (err) {
      ...
      <div className="flex flex-col flex-1 w-full lg:w-1/2">
        ...
        <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
          <div className="mb-5 sm:mb-8">
            <h1 className="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">郵件已發送</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              如果該電子郵件地址存在於我們的系統中，我們已發送密碼重置連結。
              請檢查您的收件匣。
            </p>
          </div>
          <div>
  ...
  return (
    <div className="flex flex-col flex-1 w-full lg:w-1/2">
      ...
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
        <div className="mb-5 sm:mb-8">
          <h1 className="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">忘記密碼</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">
